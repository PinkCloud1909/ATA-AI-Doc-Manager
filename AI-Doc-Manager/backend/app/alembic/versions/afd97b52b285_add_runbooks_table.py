...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['created_by'], ['users.id'], name=op.f('fk_runbooks_created_by_users')),
        sa.PrimaryKeyConstraint('id', name=op.f('runbooks_pkey'))
    )
    op.create_index(op.f('ix_runbooks_created_by'), 'runbooks', ['created_by'], unique=False)
